...
namespace LR_1
{
    class Program
    {
        static int[] position = { 0, 1 }; //Положение наблюдателя position[0] - width, position[1] - height
        static int rout = 1; // 0 - North, 1 - South, 2 - West, 3 - East.
        static int[] extremeWidth = { 0, 0 }; //Extreme positions
        static int[] extremeHeight = { 0, 0 }; //Extreme positions

        static void Main(string[] args)
        {
            int count;

            using (StreamReader streamReader = File.OpenText("../../Test.txt"))
            {
        ...
        static void Case(string road)
        {
            ...
            char[] roadExit = road.Substring(road.IndexOf(' ') + 1).ToCharArray();
